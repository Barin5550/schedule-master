...
- **Привычки**: `useHabits()` (`src/hooks/useHabits.ts`) — демо `localStorage` (`schedulemaster_habits`), реальный режим — таблица `habits` (`done_today` выводится из `last_completed === сегодня`; новому юзеру засеваются дефолтные привычки). Используется в `HabitsCard`.
- **Профиль**: `useProfile()` (`src/hooks/useProfile.ts`) — демо `localStorage` (`schedulemaster_profile`), реальный режим — таблица `user_profiles`. «Личные данные» в `SettingsAccordion` (имя/цель/продуктивное время) грузятся и сохраняются через него. Остальные настройки (шрифт, плотность, уведомления, рабочие часы) остаются клиентскими — в БД не нужны.
