...
export default function DashboardPage() {
  ...
  async function toggleTask(id: string) {
    await mutate((prev) => prev.map((t) => t.id === id ? { ...t, isCompleted: !t.isCompleted } : t));
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          footer={
            <div className="flex flex-col gap-1.5">
              ...
              <span className="text-xs text-brand-muted">{completed} из {total} выполнено</span>
            </div>
          }
        />
        <StatCard
        ...
        <StatCard
          ...
          footer={<span className="text-xs text-brand-muted">дней подряд</span>}
        />
        <StatCard
          index={3}
